...
* Widgets exclusively use swiftUI, conform to the widget protocol
* Complications can be built with swiftUI
* List: now can make outlines. Just provide a key path to a property for children of the items
...
* Create toolbars using the `.toolbar {}` modifier, with toolbarItem and Button
  ...
  * Buttons have a `.help` modifier, setting tooltips, or accessibility labels on mobile
  * Can set a `.keyboardShortcut` modifier on a Button
  * ProgressView, linear and circular
  * Gages: display a ranged value. Only shown for watch, might be elsewhere?
