...
#
# Needs a modern version, run like `RBENV_VERSION=3.3.0 ./generate_colors.rb`
...
def liquid_vars
  <<~LQ
  params:
    color_names: [#{COLORS.join(',')}]
    gray_levels_light: [#{LIGHT_GRAY_LEVELS.values.join(',')}]
    gray_levels_dark: [#{DARK_GRAY_LEVELS.values.join(',')}]
    all_levels: [#{STEPS.join(',')}]
  LQ
end
...
if __FILE__ == $PROGRAM_NAME
  ...
  replace_section('_index.md', 'USAGE TABLE', markdown_usage_table)
  replace_section('_index.md', 'EXPANDED TABLE', markdown_table(colors))

  replace_section('preview.md', 'LIQUID VARS', liquid_vars)

  replace_section('../../../assets/css/colors.css', 'MIST COLORS', css_variables(colors))
  replace_section('../../../assets/css/colors.css', 'LIGHT ALIASES',
                  css_aliases(LIGHT_GRAY_LEVELS, LIGHT_ACCENT_LEVELS, DARK_GRAY_LEVELS, DARK_ACCENT_LEVELS))
  replace_section('../../../assets/css/colors.css', 'DARK ALIASES',
